...
export const AXIS_INFO = {
  ...
  rhythm:      {label:"節奏", levels:["長音", "四分", "八分", "附點", "十六分", "切分"]},
  texture:     {label:"織度", levels:["單手", "雙手長音", "雙手塊狀", "雙手分解", "兩手齊動", "對位"]},
...
export const RHYTHM_DENSITIES = ["long", "quarter", "eighth", "varied", "16th", "16th"];
...
const PRESETS = [
  ...
  {pitchRange:4,keySignature:4,rhythm:4,texture:4,eyeHand:3,tempo:4},
  {pitchRange:5,keySignature:5,rhythm:5,texture:5,eyeHand:4,tempo:5},
];

function clamp(value){ return Math.max(0, Math.min(5, Math.round(Number(value) || 0))); }
...
export function normaliseVector(value, fallbackLevel = 1){
  ...
  for (const axis of AXES) if (value?.[axis] != null) base[axis] = clamp(value[axis]);
  return base;
}
...
export function stepVector(value, axis, direction){
  ...
  next[axis] = clamp(next[axis] + (direction === "down" ? -1 : 1));
  return next;
}

export function axisValueLabel(axis, level){
  return AXIS_INFO[axis]?.levels?.[clamp(level)] || "—";
}
...
export function chooseAxis(statsRaw, vectorRaw, direction, currentAxis, cursor = 0){
  ...
  const eligible = AXES.filter((axis) => direction === "down" ? vector[axis] > 0 : vector[axis] < 5);
